...
module.exports = router => {
  router.get('/add-to-cart/:id',   function(req, res, next){
    ...
    meme.findById(productId, function(err, meme){
        if(err){
          req.flash('Error', meme.title,'Meme was not added to the cart');
            return res.redirect('/memes');
        }
        cart.add(meme, productId);
        req.session.cart = cart;
        req.flash('success', meme.title, 'added to the cart successfully');
 ...
 router.get('/deleteItem/:id',  function(req, res, next){
  ...
  req.flash('success', meme.title, 'Meme Was Deleted Successfully');
  req.session.cart = cart;
  res.redirect('/cart/')
 });
